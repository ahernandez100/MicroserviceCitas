...
namespace RecetasMicroServiceTest
{
    ...
    public class GetRecetaByIdPacienteQueryHandlerTests
    {
        private Mock<IRecetaRepository> _repositoryMock;
        private GetRecetaByIdPacienteQueryHandler _handler;

        [TestInitialize]
        public void Setup()
        {
            _repositoryMock = new Mock<IRecetaRepository>();
            _handler = new GetRecetaByIdPacienteQueryHandler(_repositoryMock.Object);
        }
        ...
        {
            // Arrange
            var request = new GetRecetaByIdPacienteQuery { codigoPaciente = 1 }; // Código de paciente que no existe
            _repositoryMock.Setup(r => r.GetByIdPaciente(1)).Returns((Receta)null); // Simula que no se encuentra la receta
        ...
        public async Task Handle_WhenRecetaExists_ReturnsReceta()
        {
            ...
            var request = new GetRecetaByIdPacienteQuery { codigoPaciente = 1 };
            _repositoryMock.Setup(r => r.GetByIdPaciente(1)).Returns(receta); // Simula que la receta existe
